...
class ResNet32x32(nn.Module): #构建的代码框架，用它生成的对象就是一个我们可以用来训练和测试的网络
    def __init__(self, block, layers, channels, groups=1, num_classes=1000, downsample='basic'):  #表示了每个网络层的结构设置num_classes=1000
        super().__init__()
        assert len(layers) == 3 #channels=96, layers=[4,4,4], num_classes=10
        self.downsample_mode = downsample
        self.inplanes = 16
        self.conv1 = nn.Conv2d(3, 16, kernel_size=(171, 1), stride=1, ########3,(171, 31)
                               padding=1, bias=False)
        self.layer1 = self._make_layer(block, channels, groups, layers[0])
        self.layer2 = self._make_layer(
            block, channels * 2, groups, layers[1], stride=2)
        self.layer3 = self._make_layer(
            block, channels * 4, groups, layers[2], stride=2)#############stride=2
        self.avgpool = nn.AvgPool2d(8) #######################################(100, 15)
        self.fc1 = nn.Linear(block.out_channels(       #设置全连接层
    ...
    def _make_layer(self, block, planes, groups, blocks, stride=1):  #制作层结构
        downsample = None
        if stride != 1 or self.inplanes != block.out_channels(planes, groups):
            if self.downsample_mode == 'basic' or stride == 1:
                downsample = nn.Sequential(  #创建一个有序的容器，神经网络模块将按照在传入构造器的顺序依次被添加到计算图中执行，同时以神经网络模块为元素的有序字典也可以作为传入参数
                    nn.Conv2d(self.inplanes, block.out_channels(planes, groups),
                              kernel_size=1, stride=stride, bias=False),
                    nn.BatchNorm2d(block.out_channels(planes, groups)),
                )
    ...
    def forward(self, x):  #100,3,32,32   10,3,200,200     x的变化：  表示了每个层之间的交互顺序和关系130,3,200,30
        x = self.conv1(x)  #100,16,32,32  2,16,32,32  10,16,200,200  2,16,200,30                130,16,32,32
        x = self.layer1(x) #100,96,32,32  2,96,32,32  10,96,200,200  2,96,200,30                130,96,32,32
        x = self.layer2(x) #100,192,16,16 2,192,16,16 10,192,100,100 2,192,100,15               130,192,16,16
        x = self.layer3(x) #100,384,8,8   2,384,8,8   10,384,50,50   2,384,100,15               130,384,8,8
        x = self.avgpool(x)#100,384,1,1   2,384,1,1   10,384,1,1     2,384,1,1                  130,384,1,1
        x = x.view(x.size(0), -1) #10,384  2,384
        return self.fc1(x), self.fc2(x)
