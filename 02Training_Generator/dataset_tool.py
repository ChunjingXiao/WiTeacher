...
#——————————————————————————————————————————————————————————————————————————————————————
def scaler(x, feature_range=(0, 1)):
    # scale to (0, 1)
    getMax = x.max()
...
def create_mnist(tfrecord_dir, mnist_dir):
    feature = sc.loadmat('datasets/data1.mat')
    data = feature['data1']
    data = np.transpose(data, axes=[0, 3, 1, 2])
    datapad = np.pad(data, [(0, 0), (0, 0), (28, 28), (113, 113)], 'constant', constant_values=0)
    datapad = datapad[:, 0:1, :, :]
    datapad = datapad.astype(np.float32)
    assert datapad.shape == (1500, 1, 256, 256)

    feature1 = sc.loadmat('datasets/label1.mat')
    label_ = feature1['label1']
    label = np.zeros(data.shape[0], dtype=np.uint8)
    for i in range(data.shape[0]):
        label[i] = label_[0][i]
    assert label.shape == (1500,) and label.dtype == np.uint8
    assert np.min(label) == 0 and np.max(label) == 9
    onehot = np.zeros((label.size, np.max(label) + 1), dtype=np.float32)
    onehot[np.arange(label.size), label] = 1.0

    with TFRecordExporter(tfrecord_dir, datapad.shape[0]) as tfr:
        order = tfr.choose_shuffled_order()
        for idx in range(order.size):
            tfr.add_image(datapad[order[idx]])
...
def create_mnistrgb(tfrecord_dir, mnist_dir, num_images=1500, random_seed=123):
    print('Loading MAT from "%s"' % mnist_dir)
    # import gzip
    # with gzip.open(os.path.join(mnist_dir, 'train-images-idx3-ubyte.gz'), 'rb') as file:
    #     images = np.frombuffer(file.read(), np.uint8, offset=16)
    feature = sc.loadmat('datasets/data1.mat')
    ...
    data = np.transpose(data, axes=[0, 3, 1, 2])
    data_ = scaler(data)
    datapad = np.pad(data_, [(0, 0), (0, 0), (28, 28), (113, 113)], 'constant', constant_values=0)
    assert datapad.shape == (1500, 3, 256, 256)
           # and datapad.dtype == np.uint8
    assert np.min(datapad) == 0 and np.max(datapad) == 1
    with TFRecordExporter(tfrecord_dir, datapad.shape[0]) as tfr:
        order = tfr.choose_shuffled_order()
        for idx in range(order.size):
            tfr.add_image(datapad[order[idx]])
...
def create_tf_data(tfrecord_dir, data_dir):

    feature = sc.loadmat('datasets/signfi/signfi_data.mat')
    data_ = feature['signfi_data']

    data = np.zeros((1250, 200, 30, 3), dtype=np.float64)
    for i in range(1250):
        data[i, :, :, :] = data_[:, :, :, i]
    del data_
    ...
    Nmax = datapad.max()
    Nmin = datapad.min()
    datapad_piex = np.round(255 * (datapad - Nmin) / (Nmax - Nmin))
    mats = datapad_piex.astype(np.uint8)
    # mats = datapad.astype(np.float32)
    # plt.plot(datapad[0,0,:200,-30:])

    feature1 = sc.loadmat('datasets/signfi/signfi_label.mat')
    label_1 = feature1['signfi_label']
    label_ = label_1.T
    del label_1
    # np.save('images_data_raw', images[:,0,:,:])
    # np.save('images_label_raw', label_)
    labels_ = []
    index = []
    for i in range(mats.shape[0]):
        if label_[0][i] == 1: #2
            index.append(i)
            labels_.append(0)
    labels = np.array(labels_, dtype=np.uint8)
    l_index = np.array(index)
    images = mats[l_index]

    assert images.shape == (1250, 3, 256, 256) and images.dtype == np.uint8
    assert labels.shape == (1250,) and labels.dtype == np.uint8

    assert np.min(labels) == 0 and np.max(labels) == 0
    onehot = np.zeros((labels.size, np.max(labels) + 1), dtype=np.float32)
    onehot[np.arange(labels.size), labels] = 1.0
